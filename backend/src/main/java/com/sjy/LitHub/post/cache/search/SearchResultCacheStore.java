...
@Slf4j
@Component
public class SearchResultCacheStore {

	private final RedisTemplate<String, String> redisTemplate;
	private final ObjectMapper objectMapper;

	public SearchResultCacheStore(
	...
	public void save(CachePolicy policy, Page<PostSummaryResponseDTO> page, Object... args) {
		...
		try {
			...
			for (PostSummaryResponseDTO dto : page.getContent()) {
				String relatedKey = CachePolicy.RELATED_SEARCH_CACHE_KEYS.createKey(dto.getPostId());
				redisTemplate.opsForSet().add(relatedKey, key);
			} // 추적을 위한 역색인 등록
		} catch (Exception e) {
	...
	} // 키에 해당하는 Redis 값을 역직렬화하여 페이지 형태로 반환
}