...
public class PostToggleController {
	...
	public BaseResponse<LikeResponseDTO> toggleLike(@PathVariable Long postId) {
		LikeResponseDTO result = toggleService.toggleLikes(postId);
		return BaseResponse.success(result);
	}

	@Operation(summary = "게시글 스크랩 토글", description = "게시글에 스크랩을 토글합니다.")
	@PostMapping("/{postId}/scrap")
	public BaseResponse<ScrapResponseDTO> toggleScrap(@PathVariable Long postId) {
		ScrapResponseDTO result = toggleService.toggleScrap(postId);
		return BaseResponse.success(result);
	}
}